...
class Board:
    ...
    def __init__(self, device="COM5", baudrate=1000000, timeout=5):
    ...
    def buf_write(self, func, data):
        ...
        print(buf)
        print("Sending data in hex format:", ' '.join(f'{byte:02X}' for byte in buf))
        self.port.write(buf)
...
def pwm_servo_test(board):
    servo_id = 1
    board.pwm_servo_set_position(0.1, [[servo_id, 1000]])
...
if __name__ == "__main__":
    ...
    board.set_motor_speed([[1, -1], [2, 0.3], [3, -0.3], [4, 0.3]])
    # send_data = struct.pack("BBBBBBB", 0xAA, 0x55, 0x03, 0x02, 0x02, 0x01, 0x08)
    # time.sleep(1)
    board.set_motor_speed([[1, 0.0], [2, 0], [3, 0], [4, 0]])
